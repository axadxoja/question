...
function QuestionCads({ question }) {
    return (
        <Link to={`/questions/${question.id}`} className="hover:shadow-xl">
            <div className="max-w-sm bg-white border-2 border-gray-300 p-6 rounded-md tracking-wide shadow-lg">
                <div id="header" className="flex items-center mb-4">
                    <img alt="avatar" className="w-20 h-20 rounded-full border-2 border-gray-300" src={question.user.profilePic} />
                    <div id="header-text" className="leading-5 ml-6 sm">
                        <h4 id="name" className="text-xl font-semibold">J{question.user.username}</h4>
